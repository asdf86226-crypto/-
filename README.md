...
## 블로그용: 그림 + 글 자동 생성 (`blog.py`)
타임랩스 영상 대신 **그림 1장 + 어울리는 블로그 글**만 만들고 싶다면:
```bash
python blog.py                          # config.json 의 prompt 로 그림+글 생성
python blog.py --prompt "귀여운 고양이 수채화"
python blog.py --no-text                # 그림만
```
`output/art.png`(그림)와 `output/blog.txt`(제목·본문·태그)가 생깁니다.
네이버 블로그 등은 자동 글쓰기가 막혀 있어, 이 둘을 **직접 붙여넣어** 발행하세요.
(워드프레스/블로거는 API로 완전 자동화도 가능 — 필요하면 확장하세요.)
